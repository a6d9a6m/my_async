...
## 5.31
### ipc
进程间通信，消息队列，共享内存，信号量，套接字，管道，信号

管道就像个字节流的水管，一个过来，一个接走
共享内存可以同时被各方关注，然后进行锁操作
## 6.1
今天主要是在搞文档，没搞完，6.2给一份总结性的
## 6.2
这个申请书不好写呀
目前对axnet整体在arceos里面的存在做了个明确，首先，axnet自身是个crate，可以上传到crate.io，与系统本身没有太多联系。然后是net.rs这是个模块，能够调用硬件，
为api提供支持 。arceos_api/src/impl里面都是api的实现，rust和C的api是不一样的，有专门的优化，api上面是ulib，对api的进一步封装，也是应用代码前的最后一步了。
最后是最上面的app 调用相应功能。

## 6.4
### rtos实时操作系统与应用程序
应用程序是有“结束”这个概念的，就是说，一个应用程序一般都是完成一定的任务，当然大一新生写的程序和实际生产的程序不能同一而论。

在第一次接触rtos（实时操作系统）时，比方说单片机里面的程序，会发现主程序是一个无限循环，对于学习C的新人简直震惊，因为一个C程序
如果有无限循环程序就无法退出，这样子的程序八成有问题。

这就要提rtos设计的初衷了，使用rost的硬件，嵌入式场景，他是不希望这个程序停止的。就拿流水线的机械臂举例，这种程序一插上电在拔电前是永不停止的。

对于rtos来说，它有一个外部的执行调度器，所以程序的退出是通过代码通知调度器终止自己。rtos的程序是通过”任务“区分开的，在C程序里可能有的功能被我们放到
另一个函数里，这个函数集成了某一项功能，在rtos里要设计成一个“任务”，就是另一个无限循环。通过通知调度器执行不同的任务。由于调度器本身的存在所以哪怕是单片机也是
可以支持并发，异步的（虽然信号检测本来好像就能一眼看出）。还有，rtos对于阻塞的任务CPU是不会像一般操作系统一般忙等待执行的，会被调度器放到一边，如果把调度器所在的这个层次看作一个
线程，那么这个多任务就像一个协程了。但是调度器一般是厂家提供的，有一个抽象的说明书，不是很友好，需要慢慢积累熟悉各类板子。还有，就是如果任务全都阻塞了呢？
那么cpu会进入空闲低消耗状态，硬件这种节约计算资源的理念和一般操作系统上cpu一直会充分利用时时刻刻接近100%使用率是不一样的。我想起自己刚拿电脑时，常常对CPU使用接近爆炸
感到惊奇，现在来看，这只是因为正常操作系统对于CPU的充分利用罢了。

## 6.5
陈老师发的新的pdf真的可以说是完全覆盖了我的工作了，今天对这位学长的工作进行研读

这份论文主要研究的是axnet里的smoltcp与lwip的集成，刨去对背景的介绍，对smoltcp，lwip的介绍，对arceos的介绍，主要的想法集中在最后
3-4章。
### lwip使用的传统网络协议栈同步模型
就是最经典的网络分层，计网天天叨叨这个，应用层，传输层，网络层，链路层，物理层。

## 6.6
天呐，这个是学长吧，诶，佩服，一周的进度就够我学好久。

今天阅读的是2023清华操作系统大实验的两位学长的成果
### lwip
